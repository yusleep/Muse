...
└────────┬─────────────────────────┘
    ...
    └────────┬────────────────────┘
         │ ← HITL #4: 最终确认
    ┌────▼────────────────────────┐
    │ Stage 6: 编译输出 Agent       │  → .docx / LaTeX / PDF
    │ (Compilation & Export)       │
    └──────────────────────────────┘
```
...
class ThesisState(TypedDict):
    ...
    # ── Stage 6: 输出 ──
    output_format: str           # "docx" | "latex" | "pdf"
    output_filepath: str
...
```python
# 方案 1: python-docx-template (推荐, 适合学校 Word 模板)
from docxtpl import DocxTemplate

def export_to_docx(state: ThesisState, template_path: str):
    """用学校 Word 模板填充 AI 生成的内容"""
        ...
        "chapters": [
            {
                ...
                "content": ch["merged_text"],
            }
            for ch in state["chapter_results"]
        ],
        "bibliography": state["bibliography_text"],
    }
    
    doc.render(context)
    doc.save("output/thesis.docx")

# 方案 2: LaTeX (适合理工科)
# Jinja2 填充 .tex 模板 → XeLaTeX 编译 (原生支持中文)
```
...
| **Word 输出** | python-docx-template (docxtpl) | 学校模板填充 |
| **LaTeX 输出** | Jinja2 + XeLaTeX | 理工科论文 |
