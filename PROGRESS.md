...
- 2026-03-12: 收紧 `search` 阶段 fan-out：本地 refs 场景将额外 query budget 降到 2、纯在线场景降到 4，并为 academic search 增加逐源耗时日志与 429 限流熔断；新增 `tests/test_stage_enhancements.py` / `tests/test_academic_search_client.py` 回归，相关测试 `62 passed, 11 subtests passed`
- 2026-03-12: 为 chapter/citation/composition ReAct agent 补齐 `context_schema`，并将 citation/research/writing/review/composition tools 的 `ToolRuntime` 注解参数化到 `AgentRuntimeContext`；真实运行 smoke 已从 `search` 推进到 `outline` 与 `chapter_subgraph`，且未再复现此前的 `PydanticSerializationUnexpectedValue(... field_name='context' ...)` warning
- 2026-03-13: 增强 free-route ReAct 诊断日志：为 `chapter_subgraph` 增加 start / invoke_return / fallback / end 日志，为 `citation_subgraph` 增加 invoke-return / finalized 状态日志，并为 citation review tools 增加 record/finalize 日志；新增 `tests/test_react_runtime_logging.py` 回归
- 2026-03-13: 长窗口真实 probe 结论已收敛：`citation_subgraph` 在 `citation react start` 之后 120 秒内未出现任何 tool/finalize 日志，当前更像卡在首轮模型响应；`chapter_subgraph` 能进入 ReAct 并调用工具，但在免费路由下出现重复 `academic_search` 查询且 240 秒内未见 `submit_result`
