...
- **状态**：进行中
- **总进度**：0/5 phases, 0/28 steps
- **当前 Wave**：Wave 1
- **下一步**：Phase 1 Step 1.1

---

## Wave 1 — Quick Wins

### Phase 1: Prompt & Logic Fixes (1/8)
- [x] Step 1.1: 重写 chapter_review_prompt — 丰富评审 rubric (~80 行)
- [ ] Step 1.2: 字数控制反馈 — 偏差 >30% 自动重试 (~20 行)
- [ ] Step 1.3: Revision 指令合并 — 同 subtask 多条 note 不覆盖 (~8 行)
- [ ] Step 1.4: Citation allowlist 硬校验 (~12 行)
- [ ] Step 1.5: References 展示优化 — 扩大 agent 可见范围 (~10 行)
- [ ] Step 1.6: Tool JSON 双重序列化修复 (~5 行)
- [ ] Step 1.7: Subtask 范围提示增强 (~5 行)
...
### Phase 2: Reliability Hardening (0/5)
- [ ] Step 2.1: ReAct 递归上限 — 累积器 + 补写缺失 subtask (~60 行)
- [ ] Step 2.2: Revision stall 检测 — 文本相似度 + 分数趋势 (~12 行)
- [ ] Step 2.3: Self-assessment 利用 — confidence 驱动优先修订 (~20 行)
